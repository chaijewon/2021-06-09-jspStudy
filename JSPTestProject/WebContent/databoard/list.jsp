   ...
   <table class="table">
     ...
     %>
           <tr>
		      ...
		      <td width=45%><a href="../main/main.jsp?mode=4&no=<%=vo.getNo()%>"><%=vo.getSubject() %></a></td>
		      <%--   화면은 4번에 해당(detail.jsp) , no => 이번호에 해당되는 데이터 출력 요청 --%>
		      <%--
		            mode는 화면 변경 ===> main.jsp
		            no는 데이터베이스에서 값을 읽어올 목적 => detail.jsp
		            ===> request에 있는 모든 데이터 공유 (include) 
		            = 후기 , 자료실 , 카페 ...
		       --%>
